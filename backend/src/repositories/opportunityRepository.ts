...
async function persistOpportunityRows(
  ...
  const profitSnapshot = await db.profitSnapshot.create({
    data: {
      productCandidateId: productCandidate.id,
      amazonMatchId: amazonMatch.id,
      currency: opportunity.profit.currency ?? 'USD',
      marketplaceKey: opportunity.profit.marketplaceKey,
      destinationMarketplaceId: opportunity.profit.destinationMarketplaceId,
      feeRateCardVersion: opportunity.profit.feeRateCardVersion,
      vatModeKey: opportunity.profit.vatModeKey,
      vatRate: opportunity.profit.vatRate === undefined ? undefined : decimal(opportunity.profit.vatRate),
      taxableSourceShipping: Boolean(opportunity.profit.taxableSourceShipping),
      sourcePriceIncludesVat: Boolean(opportunity.profit.sourcePriceIncludesVat),
      ...
      inputVatCredit: money(opportunity.profit.inputVatCredit) ?? '0.00',
      outputVatReserve: money(opportunity.profit.outputVatReserve) ?? '0.00',
