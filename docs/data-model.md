...
- `FeeRateCard`: marketplace/category/effective-date fee assumptions used for commercial seller profit calculations.
- `VatMode`: marketplace VAT/accounting mode assumptions such as gross source prices, input VAT reclaim, and output VAT reserve behavior.
- `Order.actionItems`: links eBay orders to generated `BUY` action items for local-agent purchase review.
- `SoldComp`: imported external eBay/Terapeak/Product Research sold-comparison rows used as market evidence.
...
- `ProfitSnapshot` also stores the fee-card version, VAT mode, VAT rate, marketplace/currency, input VAT credit, and output VAT reserve used for that calculation.
...
- `SoldComp` rows keep external sold-market evidence separate from own-sales `SoldWinnerSeed` rows, so replenishment learning is not polluted by generic Terapeak/eBay market imports.
